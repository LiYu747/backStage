...
import { educationAndTraining } from '@/components/Personal/TeacherEthics.js'
import { teacherMeritAssess, disciplinaryInformation } from "@/api/AssessmentInformation.js";//师德,处分
...
//报送
const targetType = ['teacherMeritAssess', 'disciplinaryInformation']
...
const getSelectEducationTeaching = async (idx) => {
    ...
    if (idx == 0) {
        ...
        if (data0.code == 200) {
            ...
            educationAndTraining.value.children[idx].tableHeader[5].opt = appellation
        }
        const { data: data0_1 } = await queryDictByType({ type: 'grade' });//结论
        if (data0_1.code == 200) {
            let appellation = data0_1.data.dictValueList.map(s => {
                return {
                    name: s.dictValue
                }
            });
    ...
    if (idx == 1) {
        const { data: data0 } = await queryDictByType({ type: 'disciplinaryOrder' });//处分
        if (data0.code == 200) {
            let appellation = data0.data.dictValueList.map(s => {
                return {
                    name: s.dictValue
            ...
            educationAndTraining.value.children[idx].tableHeader[0].opt = appellation
        }
    }
}
...
const getUserEducationTeaching = async (val) => {
    const { code, msg, result } = ({} = await teacherMeritAssess({
    ...
    console.log(code, result, "师德");
    // //获取工作经历列表
}
// 处分
const disciplinaryPage = ref(0)
const getDisciplinary = async (val) => {
    const { code, msg, result } = ({} = await disciplinaryInformation({
        teacherId: props.userTeacherId,
        page: disciplinaryPage.value,
        sort: 'createdAt,desc',
        size: 10
    }))
    if (code === 0 && result) {
    ...
    console.log(code, result, "处分");
    // //获取工作经历列表
}

onMounted(() => {
    // getSelectEducationTeaching()//教育教学下拉框查询
    getUserEducationTeaching()//师德
    getDisciplinary()//处分
})
