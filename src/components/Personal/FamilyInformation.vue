...
import { educationAndTraining } from '@/components/Personal/FamilyInformation.js'
import { familyTies } from "@/api/AssessmentInformation.js";//工作经历,交流轮岗
...
const getSelectEducationTeaching = async (idx) => {
    ...
    if (idx == 0) {
        const { data: a1 } = await queryDictByType({ type: 'familyRelation' });//称谓，关系
        if (a1.code == 200) {
            let appellation = a1.data.dictValueList.map(s => {
                return {
                    name: s.dictValue
                }
            });
        ...
        const { data } = await queryDictByType({ type: 'politicalStatus' });//政治面貌
        if (data.code == 200) {
            let appellation = data.data.dictValueList.map(s => {
                return {
                    name: s.dictValue
                }
            });
    ...
    console.log(personalOptData, "家庭信息");
}

//家庭成员
const educationPage = ref(0)
const getUserEducationTeaching = async (val) => {
    const { code, msg, result } = ({} = await familyTies({
    ...
    console.log(code, result, "家庭成员");
    // //获取工作经历列表
}

onMounted(() => {
    // getSelectEducationTeaching()//教育教学下拉框查询
    getUserEducationTeaching()//家庭成员
})
